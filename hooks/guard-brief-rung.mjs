...
export function denyReason(state, { dispatch, detail } = {}) {
  ...
  const why = {
    ...
    "repair-close-invalid": `the repair close does not name the CURRENT round, carries no reason, or names no close-authorization event ID. A close ends an authorized repair program in band; it must say which round it ends and why.`,
    "repair-close-self-authorized": `this close is not ELIGIBLE. All three conditions, in full: the \`owner_extension\` it names carries \`"authority_kind":"close"\`; that row sits at the CURRENT round with the CURRENT candidate; and NEITHER row carries a session id this program has admitted as a worker. Record it from a session holding no worker admission here. What the third condition refuses is the ADMITTED SESSION ID, never the actor behind it — session ids are supplied by the caller, so it stops a session releasing itself by name and stops nothing that supplies a name it has not used. Have a session holding no worker admission on this program record the \`owner_extension\` and the \`repair_close\`. Named plainly: \`session_id\` is caller-supplied, so this stops the honest case, not a caller that picks a name it has not used. What it buys over a deleted ledger is a legible row.`,
    "repair-close-unauthorized": `this repair close names no close-authorization row that is ELIGIBLE. All three conditions, in full: the named \`owner_extension\` carries \`"authority_kind":"close"\`; it sits at the CURRENT round with the CURRENT candidate; and neither it nor the close carries a session id this program has admitted as a worker. Record that row, then have the close name its exact event ID. Closing releases the program's global path ownership, so the ledger keeps a CLAIMED authorization beside it rather than nothing at all. Read that literally: \`session_id\` is caller-supplied, so the row records who a release SAYS it came from, never who it came from. It is a legible record, not an authenticated one.`,
    ...
    "repair-worker-verification-missing": `this session has no typed worker-verification event for the current repair receipt. TWO routes out, and both work from here: to CONTINUE the repair, have the orchestrator persist the brief and run \`confirm-repair-brief.mjs --confirm\` to mint the receipt, then run \`--verify\` from THIS session before the first source write; to ABANDON it, record an ELIGIBLE CLOSE, which ends the program and releases its paths. All three conditions: an \`owner_extension\` carrying \`"authority_kind":"close"\`, at the CURRENT round and CURRENT candidate; a \`repair_close\` naming that exact event ID; and NEITHER row from a session this program has admitted as a worker, or they are refused as \`repair-close-self-authorized\`. A repair that is going nowhere is closed, never left holding the repo.`,
    ...
    "repair-worker-path-unauthorized": `this source path is outside the exact authorized-path set carried by the current repair brief receipt. An Owner scope event ALONE will not admit this write — the check reads the receipt you already verified, and a scope event is consumed only by the NEXT round's dispatch. The full route: the Owner records an \`owner_extension\` with \`"authority_kind":"scope"\` listing the added paths, the next gate round carries that exact event ID with \`new_scope: true\` and the widened path set, its brief is confirmed into a new receipt, and this session runs \`--verify\` against that receipt. Until then the path stays out of scope, which is what the set is for.`,
