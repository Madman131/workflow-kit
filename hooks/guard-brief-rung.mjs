...
export function denyReason(state, { dispatch, detail } = {}) {
  ...
  const why = {
    ...
    "repair-worker-verification-missing": `this session has no typed worker-verification event for the current repair receipt. TWO routes out, and both work from here: to CONTINUE the repair, have the orchestrator persist the brief and run \`confirm-repair-brief.mjs --confirm\` to mint the receipt, then run \`--verify\` from THIS session before the first source write; to ABANDON it, record an \`owner_extension\` with \`"authority_kind":"close"\` at the current round and then a \`repair_close\` naming that event ID, which ends the program and releases its paths — BOTH recorded from a session this program has NOT admitted as a worker, or they are refused as \`repair-close-self-authorized\`. A repair that is going nowhere is closed, never left holding the repo.`,
