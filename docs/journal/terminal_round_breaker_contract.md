...
**Policy v3 cutover (2026-09-17).** The aggregate envelope accepts absent historical version,
explicit v2, and explicit v3 only; explicit v1 and every other value make the ledger unavailable.
The frozen `060a055f` v2 reader must reject a valid-hash v3 row before deriving usable authority.
Before any operational v3 append, inventory each participating controller, recorder, confirmer,
and guard by checkout path and SHA-256; synchronize participants or record a dormant checkout as
an excluded nonparticipant. Replay never rewrites, filters, or relabels prior ledger bytes.
...
carry is 2500, so the sole exit is never shape-impossible) · Owner evidence; or exact v3 T2 Principal evidence for reviewed `split`, `new_changeset`, or the single bounded completion child, with every child path inside the parent's opened coverage (`material_scope` stays Owner-only) | lineage ONLY — no ownership, no verdict, no dispatch; the ONE successor mechanism; anchors only to a TERMINAL parent (GO, STOP, or CLOSED — a non-terminal or held state can never seed a child) |
...
| T12 | any non-terminal | `close` | Owner- or eligible v3 T2 Principal-evidenced; session ∉ the set admitted BEFORE this row's position (as-of — a later admission never re-adjudicates it); cites the latest disposition or (if none exists) the winning open; reserves after any recorded receipt | `CLOSED` |
...
STOP, never GO · a dead worker is replaced only by an Owner- or eligible v3 T2 Principal-evidenced handoff that revokes the
