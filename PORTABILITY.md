...
- **Those files were never committed.** They are gitignored in the origin repo
  ...
  **Read that bullet precisely, because since v2.28.0 `init` gitignores ONE file of your `.codex/`
  install — `.codex/hooks.json`, and only when the kit wrote it.** The condemnation above is not
  "gitignored"; it is **gitignored and existing nowhere else**: unreviewed, out of CI, history
  unrecoverable. `.codex/hooks.json` is different in kind: it bakes **this checkout's absolute path**
  into every registered command, because Codex runs a hook from a working directory the kit does not
  control and a wrong project root is a fail-OPEN. Committed, it registers hooks at a path another
  clone or linked worktree does not have — and a hook that fails to *start* blocks nothing,
  **silently**. Its SOURCE is not lost by ignoring it: the registration is regenerated by every run
  of `init`, and the hook files it points at are tracked twice — `hooks/*.mjs` in this kit and
