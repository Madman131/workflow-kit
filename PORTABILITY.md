...
- **Those files were never committed.** They are gitignored in the origin repo
  ...
  **Read that bullet precisely, because since v2.28.0 `init` gitignores ONE file of your `.codex/`
  install — `.codex/hooks.json`, and only on a run in which `init` itself wrote it.** The condemnation
  above is not "gitignored"; it is **gitignored and existing nowhere else**: unreviewed, out of CI,
  history unrecoverable. `.codex/hooks.json` is different in kind: it bakes **this checkout's absolute
  path** into every registered command, because Codex runs a hook from a working directory the kit
  does not control and a wrong project root is a fail-OPEN. Committed, it registers hooks at a path
  another clone or linked worktree does not have — and a hook that fails to *start* blocks nothing,
  **silently**. Its SOURCE is not lost by ignoring it: the registration is regenerated by every run of
  `init`, and the hook files it points at are tracked twice — `hooks/*.mjs` in this kit and
  `.claude/hooks/*.mjs` in your repo — with `init` byte-comparing the two installed trees on every
  run. Nothing else in the lane is ignored: `.codex/config.toml` is `[P]` and path-free (and kept when
  you carry your own), `.codex/hooks/` are byte-copies of your tracked `.claude/hooks/`, and
  `.codex/agents/*.toml` is `[G]` content your team reviews. **The decision is keyed on the one fact
  the installer actually has — that it wrote the file this run.** It does not inspect a registration's
  content to guess whose it is: no stamp, path or string proves authorship, and three rounds of cold
  review showed every such guess hiding an adopter's own file. A rule written on an earlier run
  persists on its own. When `init` preserved a registration you carry (your `config.toml` declares
  hooks) while an ignore rule for it is still active, it says it cannot tell whose file that is now
  and tells you which line to delete if it is yours; it never edits `.gitignore` lines. **An ignore
  rule never untracks a file git already indexes:** a tracked copy of the registration `init` just
  wrote is a counted refusal carrying `git rm --cached -- .codex/hooks.json`. **Every clone and every
  linked worktree runs its own `init`.**
...
or provider-settings request; they do not prove a live provider. If you gate by hand, record that in `core/BINDINGS.md § Tool bindings` and treat
