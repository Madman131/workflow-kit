...
```
  ...
  - **`--disable code_mode_host` — DO NOT PASS IT, and grep your runners for it too.** Measured 2026-08-12 on **0.147.0-alpha.6.5**, one variable changed: with the flag the seat **could not open the file it was asked for** (router: `error=code-mode host is disabled`) and **the run still exited 0**; without it, same prompt, model, repo and sandbox, it quoted the requested line byte-for-byte. `codex features list` reports `code_mode_host` as `stable`, default ON. *(Found first by the `llm-peripheral-brain` lane on 0.147.0-alpha.6.5 and re-measured independently here.)* **`grep -n code_mode_host scripts/codex-gate.sh scripts/sweep.mjs` must return nothing** — it reached three call sites in this repo, and a fourth in the sweep sensor, by being copied rather than typed. *History, not instruction:* 0.144-alpha REQUIRED it (a missing helper binary killed every run), 0.145-alpha.18 was indifferent (verified 2026-07-17), and it was then kept as cross-version insurance. **The general rule this incident bought — on relying at all on version-pinned tool behaviour — is `core/WORKFLOW.md` § Gate. It is a RULE, not an invocation detail, so it is NOT restated here: go and read it.** *(Deliberately a pointer and not a copy. A second copy is a second thing to keep in step, and a stale mirror of a rule is worse than no mirror — this file's own class declaration is what puts the rule there.)* **If a future CLI needs this flag again, re-add it only with a canary proving the seat can still quote a file it was NOT given, and record the version you proved it on.** Symptom and detection: § Gotchas / traps.
...
- **Per-round receipt, both modes:** each invocation appends a fresh random token the reviewer must
  ...
  **that cites evidence the payload did not contain.** ⚠ **The first three properties are exactly
  what a BLINDED seat still produces** (§ Gotchas / traps): it has the payload, so it is real, on
  topic and ranked, and resting on nothing. **Only the last discriminates** — so check it by the
  method in § Gotchas / traps (a line from a file that was *not* in the payload), not by reading the
  verdict and finding it convincing. **A blinded seat's verdict is convincing; that is the problem.**
  **Bound on this test:** it catches a seat that could not READ. It does **not** detect a silent
...
- **A BLINDED SEAT EXITS 0, AND ITS VERDICT IS REAL, ON-TOPIC AND SEVERITY-RANKED.** Losing file access does not stop a run: the payload still reaches the seat over the prompt, so it answers from that alone — fluent, well-formed, and resting on nothing it could independently check. **This is a FAIL-OPEN, not a breakage: no exit code, no receipt and no verdict-shape check distinguishes it from a healthy gate.** **MEASURED 2026-08-12, not inferred** — paired planted-discrepancy runs, **n=3 per arm, all six valid, one variable**: both arms got a payload-complete review packet in which one quoted constant was false against the file on disk. **The sighted arm caught it 3/3, quoting the real line exactly. The blinded arm confirmed the false claim `HOLDS` 3/3, with no hedge and no mention of read failure anywhere in its output.** ⚠ **AND THE PART THAT MAKES IT INVISIBLE: the blinded seat hedges perfectly well — it returned `UNVERIFIED` 3/3 on a different claim whose excerpt the packet visibly omitted. Its hedging tracks PACKET completeness, not SOURCE access.** So *"the seat would have said something"* is false: it says something precisely when the gap is visible in the packet, and says nothing when the gap is only visible against disk. ⇒ **NEVER WRITE THIS CHECK AS "LOOK FOR HEDGING". Hedging is present, it is accurate, and it is not the signal** — a seat that hedges fluently about packet gaps reads as careful, which is worse than one that says nothing at all. **The signal is a specific claim about content the payload did not carry, checked by you against the file.** ⚠ **And do not take the seat's account of its own work as evidence either: one blinded run opened by narrating *"checking the contract directly against the current source"*, which it had not done and could not do.** A seat can confabulate the PROCESS as readily as the finding, so **any control that reads a seat's self-description of its method — an `INSPECTED SCOPE:` line, a "verified against" narration — is satisfied perfectly by the failure it is meant to catch.** Check the artifact, never the account. *Bound on this evidence: a planted falsehood is a stronger test than a live review — it proves the CAPABILITY is gone, not that blinding harms a review with no planted defect.* `--disable code_mode_host` did exactly this (§ How to run), and **the flag can hide in a runner you copied rather than a command you typed** — `grep -n code_mode_host scripts/codex-gate.sh scripts/sweep.mjs` must return nothing. **DETECT IT:** ask the seat to quote an exact line of a file **that was not in its payload**, and compare it against the file yourself. ⚠ **A file from the changeset does not work** — the packet contract carries every changed file in full, so a blinded seat quotes it from what it was handed and passes your check. The router's error goes to **stderr**, so it is captured only if you redirect it; on stdout the run looks clean. **A run detected blind is VOID: re-run it cold, never `--resume` it** — a warm round inherits the thread, not the eyesight, so resuming launders the same blindness into a second verdict. **The sweep sensor fails harder and quieter:** its prompt carries file PATHS, not contents, so a blinded sweep seat has nothing whatever to read, yet can still attest `COVERAGE: <file> :: scanned` for every file and exit 0 — a vacuous clean report that the wrapper cannot contradict. ⚠ **THIS IS A FAMILY, NOT AN ANECDOTE — THE FAILURE STATE PRODUCING THE SAME ARTIFACT AS SUCCESS.** The governing rule is `core/WORKFLOW.md` § Gate (*a blind verification tool returns success*); what belongs here is the count, because the count is the argument. **Four instances surfaced in a single day, on four different surfaces:** a blinded seat returning a fluent verdict at exit 0 · a dependency sweep that would have run vacuous-but-green · **a shell command that failed and still produced a commit** (`head -n -2`, unsupported on BSD, which silently truncated a commit message to a fragment and committed it — the build step errored, the artifact appeared, and only the verify step caught it) · and a `grep` run as a claim-check, returning a count that read as a clean result while counting the wrong thing. **None announced itself. Each was found by a later step that happened to look.** ⇒ **Ask of every step, not only of seats: if this had silently failed, would its output be distinguishable from success?**
...
Slicing is a coverage strategy, not an automatic remediation loop. The active workhorse PM approves the plan before model calls.
