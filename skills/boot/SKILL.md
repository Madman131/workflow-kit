...
1. **Identity.** Run `git remote get-url origin` and compare it to the fingerprint your entry
   stub records — never trust a path. A mismatch means the wrong checkout: STOP.
...
3. **Other-lane check.** `git status --short` — unexplained dirty files are another lane's
   work, untouchable. Read `.claude/task-lane.json`. **Unsure whether another lane is live —
   including an absent or stale declaration — use a private worktree: fail closed**
   (`core/MULTI_AGENT.md` § Multi-writer checkout).
