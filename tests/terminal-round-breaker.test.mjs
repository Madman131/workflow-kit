...
function processReview(ctx, closeId, candidate, ruling = "finish_bounded_root") {
  ...
  return recordAggregateProcessReview({
    ...
    anchor: { kind: "aggregate_panel_close", event_id: closeId,
      frozen_commit: candidate.commit, frozen_tree: candidate.tree },
    proposed_transition: { policy_version: state.policy_version === 3 ? 3 : AGGREGATE_POLICY_VERSION,
...
function acceptedV3Root(ctx, candidate) {
  const minted = openPanel(ctx, 1, candidate);
  assert.equal(minted.opened.ok, true, minted.opened.state);
  const root = stamped({ ...loadRepairEventsForProject(ctx.dir).aggregate_events[0].event,
    policy_version: 3 });
  writeFileSync(repairLedgerPath(ctx.dir), `${JSON.stringify(root)}\n`);
  assert.equal(derive(ctx).panels_open[0].event_id, root.event_id,
    "the accepted historical T2 root survives the reader upgrade");
  return { opened: { ok: true, event_id: root.event_id }, expected: minted.expected };
}
...
test("M54: a Project Principal can bind only the external review seat, never the Builder-family panel or Owner paths", () => {
  const principalSubstitution = (seat, actualFamily = "gemini", level = "full") => ({
    ...
    architect_evidence: {
      authority_record: "program-record:principal-1", decision_id: "principal-seat-1",
      scope: "review-seat-substitution", seat_id: seat.seat_id, replaced_family: seat.family,
      actual_family: actualFamily, decorrelation_level: level,
      provider: "google", model: "gemini-3.1-pro-high",
    },
  });
  const attempt = (change) => {
    const ctx = repo();
    try {
      const candidate = commit(ctx.dir, 54);
      const seats = expectedSeats(candidate.paths);
      change(seats, principalSubstitution);
      return recordAggregatePanelOpen({
        type: "aggregate_v2", kind: "panel_open", task_id: "task-54", changeset_id: "cs-54", round: 1,
        phase: "repair_round", tier: "T2", frozen_commit: candidate.commit, frozen_tree: candidate.tree,
        base_ref: "origin/main", base_commit: ctx.base, expected_seats: seats,
  ...
  const replaceExternal = (seats, substitution) => {
  ...
  for (const [name, mutate] of [
    ...
    ["mixed Owner and principal evidence", (seat) => { seat.substitution.owner_evidence = "Owner"; }],
  ]) {
    const result = attempt((seats, substitution) => mutate(replaceExternal(seats, substitution)));
    assert.equal(result.ok, false, `${name} cannot mint a principal substitution`);
  }
  ...
  assert.equal(attempt((seats, substitution) => {
    replaceExternal(seats, (seat) => substitution(seat, "codex", "one-family"));
...
test("M55: architect evidence cannot replace Owner evidence on terminal child continuation", () => {
  const ctx = repo();
  try {
    const candidate = commit(ctx.dir, 55);
    const panel = openPanel(ctx, 1, candidate);
    assert.equal(panel.opened.ok, true, panel.opened.state);
    const closed = closePanel(ctx, panel, candidate, ["F55"]);
    const stopped = decide(ctx, closed, { accepted: ["F55"], terminal_state: "STOP",
    ...
    const continuation = recordAggregateChildContinuation({
      ...
      parent_disposition_event_id: stopped.event_id, trigger_ids: ["F55"],
      continuation_kind: "new_changeset", architect_evidence: { scope: "review-seat-substitution" },
      children: [{ task_id: "child-55", changeset_id: "child-55-cs", tier: "T2",
        budget: "one bounded child", authorized_paths: ["src/x.mjs"] }],
    }, options(ctx.dir));
    assert.equal(continuation.ok, false,
      "terminal continuation still requires actual Owner evidence, never principal review evidence");
  } finally { ctx.cleanup(); }
});
...
test("a bare accepted v3 T2 parent cannot mint a new T3 child", () => {
  const ctx = repo();
  try {
    const candidate = commit(ctx.dir, 1);
    const panel = acceptedV3Root(ctx, candidate);
    const closed = closePanel(ctx, panel, candidate, ["F1"]);
    assert.equal(closed.ok, true, closed.state);
    const stopped = decide(ctx, closed, { accepted: ["F1"], terminal_state: "STOP",
      remediation_kind: null, authorized_paths: [] });
    assert.equal(stopped.ok, true, stopped.state);
    const before = readFileSync(repairLedgerPath(ctx.dir), "utf8");
    const child = recordAggregateChildContinuation({ type: "aggregate_v2", kind: "child_continuation",
      task_id: "task-1", changeset_id: "cs-1", parent_disposition_event_id: stopped.event_id,
      trigger_ids: ["F1"], continuation_kind: "new_changeset",
      owner_evidence: "Owner text cannot grandfather a new T3 classification",
      children: [{ task_id: "new-t3-child", changeset_id: "new-t3-child-cs", tier: "T3",
        budget: "one change", authorized_paths: candidate.paths }] }, options(ctx.dir));
    assert.equal(child.ok, false, "historical T2 provenance alone cannot mint new T3");
    assert.equal(readFileSync(repairLedgerPath(ctx.dir), "utf8"), before);
  } finally { ctx.cleanup(); }
});

test("a v3 T2 parent cannot promote its next newly opened panel to T3", () => {
  const ctx = repo();
  try {
    const first = commit(ctx.dir, 1);
    const panel = acceptedV3Root(ctx, first);
    const closed = closePanel(ctx, panel, first, ["F1"]);
    const decided = decide(ctx, closed, { accepted: ["F1"] });
    const review = processReview(ctx, closed.event_id, first);
    const authority = dispatchBatch(ctx, decided.event_id, closed.event_id, 2, null,
      { processReviewId: review.event_id });
    const second = commit(ctx.dir, 2);
    const before = readFileSync(repairLedgerPath(ctx.dir), "utf8");
    const promoted = openPanel(ctx, 2, second, { tier: "T3",
      incoming: { dispatch: authority.dispatch, worker: authority.worker } });
    assert.equal(promoted.opened.ok, false, "inherited v3 policy cannot mint a new T3 panel");
    assert.equal(readFileSync(repairLedgerPath(ctx.dir), "utf8"), before);
    const continued = openPanel(ctx, 2, second, { tier: "T2",
      incoming: { dispatch: authority.dispatch, worker: authority.worker } });
    assert.equal(continued.opened.ok, true, continued.opened.state);
    const rows = loadRepairEventsForProject(ctx.dir).aggregate_events;
    const historical = stamped({ ...rows.at(-1).event, tier: "T3",
      expected_seats: expectedSeats(second.paths, "T3") });
    writeFileSync(repairLedgerPath(ctx.dir),
      `${[...rows.slice(0, -1), historical].map(JSON.stringify).join("\n")}\n`);
    assert.equal(derive(ctx).tier, "T3", "a recorded historical escalation still replays");
    const collected = closePanel(ctx, { opened: { event_id: historical.event_id },
      expected: historical.event.expected_seats }, second, ["F2"]);
    assert.equal(collected.ok, true, collected.state);
  } finally { ctx.cleanup(); }
});

test("a recorded v3 T2-to-T3 completion keeps its pending child and worker at full T3 strength", () => {
  const ctx = repo();
  try {
    let candidate = commit(ctx.dir, 1);
    let panel = acceptedV3Root(ctx, candidate);
    let stopped = null;
    for (let round = 1; round <= 4; round++) {
      const closed = closePanel(ctx, panel, candidate, [`F${round}`]);
      assert.equal(closed.ok, true, closed.state);
      stopped = decide(ctx, closed, { accepted: [`F${round}`],
        terminal_state: round === 4 ? "STOP" : "CONTINUE",
        remediation_kind: round === 4 ? null : round >= 2 ? "root_replacement" : "bounded",
        authorized_paths: round === 4 ? [] : ["src/x.mjs"] });
      assert.equal(stopped.ok, true, stopped.state);
      ...
      let root = null;
      if (round >= 2) {
        root = recordAggregateRootExit({ type: "aggregate_v2", kind: "root_exit",
          task_id: "task-1", changeset_id: "cs-1", disposition_event_id: stopped.event_id,
          shared_mechanism: "one historical defect", symptom_explanation: "prior repairs treated symptoms",
          owner_state_yield_seams: ["owner seam"], replacement: "one correction",
          removed_workarounds: ["repeat patch"], trigger_matrix: ["original trigger closes"],
          closure_evidence: "candidate closes the trigger" }, options(ctx.dir));
      ...
      const review = round === 3 ? processReview(ctx, closed.event_id, candidate) : null;
      if (review) assert.equal(review.ok, true, review.state);
      const authority = dispatchBatch(ctx, stopped.event_id, closed.event_id, round + 1,
        root?.event_id ?? null, { processReviewId: review?.event_id ?? null });
      candidate = commit(ctx.dir, round + 1);
      panel = openPanel(ctx, round + 1, candidate, { tier: "T2",
        incoming: { dispatch: authority.dispatch, worker: authority.worker } });
      assert.equal(panel.opened.ok, true, panel.opened.state);
    }
    mkdirSync(path.join(ctx.dir, "briefs"), { recursive: true });
    writeFileSync(path.join(ctx.dir, "briefs/completion.md"), "one historical completion repair\n");
    const completion = { type: "aggregate_v2", kind: "child_continuation", task_id: "task-1",
      changeset_id: "cs-1", parent_disposition_event_id: stopped.event_id, trigger_ids: ["F4"],
      continuation_kind: "completion_exception", owner_evidence: "Owner authorized T3 completion",
      children: [{ task_id: "v3-t3-completion", changeset_id: "v3-t3-completion-cs", tier: "T3",
        budget: "one final batch", authorized_paths: [...candidate.paths, "briefs/completion.md"].sort() }],
      completion_exception: { repair_batches: 1, final_panels: 1,
        final_panel: { phase: "final_bookend", tier: "T3", coverage: "full" },
        pm_recommendation: "one correction", surviving_harm: "F4", smallest_correction: "src/x.mjs",
        completion_proof: "full final panel" },
      completion_batch: { worker_session_id: "v3-completion-worker", brief_path: "briefs/completion.md" } };
    const parent = derive(ctx);
    const lastOpen = parent.panels_open.at(-1);
    const briefBytes = readFileSync(path.join(ctx.dir, completion.completion_batch.brief_path));
    const historicalProposal = { policy_version: 3, authority_route: "owner",
      action_screen: _DEFAULT_CONTINUATION_SCREEN, ...completion,
      completion_batch: { ...completion.completion_batch,
        brief_sha256: createHash("sha256").update(briefBytes).digest("hex"), brief_size: briefBytes.length } };
    const beforeReview = readFileSync(repairLedgerPath(ctx.dir), "utf8");
    const newReview = recordAggregateProcessReview({ type: "aggregate_v2", kind: "process_review",
      task_id: "task-1", changeset_id: "cs-1", reviewer_role: "frontier",
      purpose: "child_continuation", proposed_transition: historicalProposal,
      anchor: { kind: "aggregate_terminal", event_id: stopped.event_id,
        panel_open_event_id: lastOpen.event_id, frozen_commit: lastOpen.frozen_commit,
        frozen_tree: lastOpen.frozen_tree }, review_evidence: "new T3 proposal",
      zoom_out: "one bounded exit", ruling: "owner_decision", bounded_scope: "one final repair",
      closure_evidence: "F4 closes" }, options(ctx.dir));
    assert.equal(newReview.ok, false, "a current review cannot authorize new T3 from bare T2");
    assert.equal(readFileSync(repairLedgerPath(ctx.dir), "utf8"), beforeReview);
    const review = stamped({ type: "aggregate_v2", policy_version: 3, kind: "process_review",
      task_id: "task-1", changeset_id: "cs-1", recorded_at: "2099-01-01T00:00:00.000Z",
      session_id: "historical-frontier", reviewer_role: "frontier",
      purpose: "child_continuation",
      anchor: { kind: "aggregate_terminal", event_id: stopped.event_id,
        panel_open_event_id: lastOpen.event_id, frozen_commit: lastOpen.frozen_commit,
        frozen_tree: lastOpen.frozen_tree },
      transition_sha256: aggregateTransitionSha256("child_continuation", historicalProposal),
      next_gate_ordinal: 5, review_evidence: "Owner completion reviewed",
      zoom_out: "one bounded exit", ruling: "owner_decision", bounded_scope: "one final repair",
      closure_evidence: "F4 closes" });
    writeFileSync(repairLedgerPath(ctx.dir), `${JSON.stringify(review)}\n`, { flag: "a" });
    assert.equal(derive(ctx).process_reviews.some((row) => row.event_id === review.event_id), true,
      "the accepted historical review remains readable");
    const before = readFileSync(repairLedgerPath(ctx.dir), "utf8");
    const newChild = recordAggregateChildContinuation({ ...completion,
      process_review_event_id: review.event_id }, options(ctx.dir));
    assert.equal(newChild.ok, false, "an old T2 parent cannot issue a new T3 completion authorization");
    assert.equal(readFileSync(repairLedgerPath(ctx.dir), "utf8"), before);
    // This accepted row is pre-recorded historical authority. Replay must keep the pending T3
    // child while the current recorder refuses an otherwise identical NEW authorization.
    const child = stamped({ type: "aggregate_v2", policy_version: 3, kind: "child_continuation",
      task_id: "task-1", changeset_id: "cs-1", recorded_at: "2099-01-01T00:00:01.000Z",
      session_id: "historical-owner", parent_disposition_event_id: stopped.event_id,
      parent_frozen_commit: lastOpen.frozen_commit, parent_frozen_tree: lastOpen.frozen_tree,
      trigger_ids: completion.trigger_ids, continuation_kind: completion.continuation_kind,
      children: completion.children, authority_route: "owner", owner_evidence: completion.owner_evidence,
      action_screen: _DEFAULT_CONTINUATION_SCREEN, completion_exception: completion.completion_exception,
      completion_batch: { worker_session_id: completion.completion_batch.worker_session_id,
        brief_path: completion.completion_batch.brief_path,
        brief_sha256: createHash("sha256").update(briefBytes).digest("hex"), brief_size: briefBytes.length,
        authorized_paths: completion.children[0].authorized_paths },
      process_review_event_id: review.event_id });
    writeFileSync(repairLedgerPath(ctx.dir), `${JSON.stringify(child)}\n`, { flag: "a" });
    assert.equal(derivePendingLineageBudgets(loadRepairEventsForProject(ctx.dir).aggregate_events)
      .some((entry) => entry.task_id === "v3-t3-completion"), true,
    "the pre-recorded T3 child remains an accepted pending lineage");
    const worker = recordWorkerVerification({ task_id: "v3-t3-completion",
      repair_dispatch_event_id: child.event_id }, options(ctx.dir, "v3-completion-worker"));
    assert.equal(worker.ok, true, worker.state);
    const loaded = loadRepairEventsForProject(ctx.dir);
    assert.equal(loaded.aggregate_events.at(-1).event.policy_version, 3,
      "the completion worker inherits the child's accepted historical version");
    assert.deepEqual(derivePendingLineageBudgets(loaded.aggregate_events)
      .map((entry) => entry.task_id), ["v3-t3-completion"]);
    const finalCandidate = commit(ctx.dir, 5);
    const final = recordAggregatePanelOpen({ type: "aggregate_v2", kind: "panel_open",
      task_id: "v3-t3-completion", changeset_id: "v3-t3-completion-cs", round: 1,
      phase: "final_bookend", tier: "T3", frozen_commit: finalCandidate.commit,
      frozen_tree: finalCandidate.tree, base_ref: "origin/main", base_commit: ctx.base,
      expected_seats: expectedSeats(finalCandidate.paths, "T3"),
      incoming_dispatch_event_id: null, incoming_worker_event_id: worker.event_id,
    ...
    assert.equal(derive(ctx, "v3-t3-completion").panels_open[0].event_id, final.event_id);
  } finally { ctx.cleanup(); }
});

test("tierless standard history refuses new T3 while a recorded T3 handoff still progresses", () => {
  const ctx = repo();
  try {
    const candidate = commit(ctx.dir, 1);
    assert.equal(openPanel(ctx, 1, candidate, { task: "fresh-t3", changeset: "fresh-t3-cs",
      tier: "T3" }).opened.state, "aggregate-panel-open-malformed",
    "a fresh v4 T3 root remains refused");
    const manifest = fingerprintCandidate(ctx.dir, candidate.paths);
    const parent = { type: "round_disposition", task_id: "standard-parent",
      ...
      finding_ids: ["LEGACY-F1"], finding_class: "legacy", ownership_area: "controller",
      original_trigger: "active historical standard work", authorized_paths: candidate.paths,
    ...
    const ledger = repairLedgerPath(ctx.dir);
    mkdirSync(path.dirname(ledger), { recursive: true });
    writeFileSync(ledger, `${JSON.stringify(stamped(parent))}\n`);
    assert.equal(loadRepairEventsForProject(ctx.dir).events.length, 1,
      "the standard parent is accepted history, not a fresh standard mint");
    const handoffInput = { type: "aggregate_v2", kind: "legacy_handoff",
      ...
      child: { task_id: "historical-standard-child", changeset_id: "historical-standard-child-cs",
        tier: "T3", budget: "one Owner-authorized continuation", authorized_paths: candidate.paths },
      owner_evidence: "Owner authorized historical T3 handoff" };
    const before = readFileSync(ledger, "utf8");
    const newHandoff = recordAggregateLegacyHandoff(handoffInput, options(ctx.dir));
    assert.equal(newHandoff.ok, false,
      "tierless standard history and Owner text cannot create a new T3 classification");
    assert.equal(readFileSync(ledger, "utf8"), before);
    const handoff = stamped({ ...handoffInput, policy_version: 3,
      recorded_at: "2099-01-01T00:00:01.000Z", session_id: "historical-owner",
      parent_disposition_event_id: eventId(parent), parent_round: 1,
    ...
    assert.deepEqual(derivePendingLineageBudgets(loaded.aggregate_events, { standardEvents: loaded.events })
      .map((entry) => entry.task_id), ["historical-standard-child"]);
    const next = commit(ctx.dir, 2);
    const child = openPanel(ctx, 1, next, { task: "historical-standard-child",
      changeset: "historical-standard-child-cs", tier: "T3",
      lineage: { handoff: handoff.event_id } });
    assert.equal(child.opened.ok, true, child.opened.state);
    assert.equal(derive(ctx, "historical-standard-child").panels_open[0].event_id, child.opened.event_id);
  } finally { ctx.cleanup(); }
});

test("tierless standard history permits only an explicit safeguarded T2 handoff", () => {
  const ctx = repo();
  try {
    const candidate = commit(ctx.dir, 1);
    const manifest = fingerprintCandidate(ctx.dir, candidate.paths);
    const parent = { type: "round_disposition", task_id: "standard-t2",
      changeset_id: "standard-t2-cs", round: 1, candidate_sha: manifest.digest,
      candidate_manifest: manifest.records, verdict: "NO-GO", disposition: "REMEDIATE",
    ...
    mkdirSync(path.dirname(ledger), { recursive: true });
    writeFileSync(ledger, `${JSON.stringify(stamped(parent))}\n`);
    const handoff = { type: "aggregate_v2", kind: "legacy_handoff", task_id: "standard-t2",
      changeset_id: "standard-t2-cs", parent_task_id: "standard-t2",
      parent_changeset_id: "standard-t2-cs", parent_candidate_sha: manifest.digest,
      authorized_paths: candidate.paths,
      child: { task_id: "new-t2-child", changeset_id: "new-t2-child-cs", tier: "T2",
        budget: "one exact child", authorized_paths: candidate.paths },
      owner_evidence: "Owner explicitly authorizes this T2 handoff" };
    const before = readFileSync(ledger, "utf8");
    assert.equal(recordAggregateLegacyHandoff({ ...handoff, owner_evidence: undefined },
      options(ctx.dir)).ok, false, "Owner evidence is required");
    assert.equal(recordAggregateLegacyHandoff({ ...handoff,
      child: { ...handoff.child, authorized_paths: ["other/path.mjs"] } }, options(ctx.dir)).ok,
    false, "the child cannot silently widen scope");
    assert.equal(readFileSync(ledger, "utf8"), before);
    const minted = recordAggregateLegacyHandoff(handoff, options(ctx.dir));
    assert.equal(minted.ok, true, minted.state);
    const loaded = loadRepairEventsForProject(ctx.dir);
    assert.equal(loaded.aggregate_events.at(-1).event.policy_version, 4);
    assert.deepEqual(derivePendingLineageBudgets(loaded.aggregate_events, { standardEvents: loaded.events })
      .map((entry) => entry.task_id), ["new-t2-child"]);
    const next = commit(ctx.dir, 2);
    const child = openPanel(ctx, 1, next, { task: "new-t2-child", changeset: "new-t2-child-cs",
      tier: "T2", lineage: { handoff: minted.event_id } });
    assert.equal(child.opened.ok, true, child.opened.state);
  } finally { ctx.cleanup(); }
});
...
test("an old cross-envelope T3 handoff replays at T3 strength while current admission refuses", () => {
  ...
  try {
    ...
    execFileSync("git", ["commit", "-qm", "unrelated program"], { cwd: ctx.dir });
    const unrelated = { commit: execFileSync("git", ["rev-parse", "HEAD"], { cwd: ctx.dir,
      encoding: "utf8" }).trim(), tree: execFileSync("git", ["rev-parse", "HEAD^{tree}"], {
      cwd: ctx.dir, encoding: "utf8" }).trim(), paths: ["src/y.mjs"] };
    const root = openPanel(ctx, 1, unrelated, { task: "unrelated", changeset: "unrelated-cs", tier: "T2",
      baseCommit: standardCandidate.commit });
    assert.equal(root.opened.ok, true, root.opened.state);
    const rows = loadRepairEventsForProject(ctx.dir).aggregate_events;
    const historical = stamped({ ...rows.at(-1).event, policy_version: 3, tier: "T3",
      expected_seats: expectedSeats(unrelated.paths, "T3") });
    writeFileSync(ledger, `${JSON.stringify(parent)}\n${JSON.stringify(historical)}\n`);
    assert.equal(derive(ctx, "unrelated").tier, "T3");
    const input = { type: "aggregate_v2", kind: "legacy_handoff", task_id: "unrelated",
      changeset_id: "unrelated-cs", parent_task_id: "standard-parent",
      parent_changeset_id: "standard-parent-cs", parent_candidate_sha: manifest.digest,
      authorized_paths: standardCandidate.paths,
      child: { task_id: "borrowed-t3", changeset_id: "borrowed-t3-cs", tier: "T3",
        budget: "one handoff", authorized_paths: standardCandidate.paths },
      owner_evidence: "Owner handoff for standard parent" };
    const before = readFileSync(ledger, "utf8");
    assert.equal(recordAggregateLegacyHandoff(input, options(ctx.dir)).ok, false,
      "the aggregate envelope is not evidence of the standard parent's historical tier");
    assert.equal(readFileSync(ledger, "utf8"), before);
    const planted = stamped({ ...input, policy_version: 3,
      parent_disposition_event_id: parent.event_id, parent_round: 1,
      process_review_event_id: null, recorded_at: "2099-01-01T00:00:01.000Z",
      session_id: "cross-parent-writer" });
  ...
  for (const [policy, ruling] of [[2, "owner_decision"], [2, "successor"],
    ...
    try {
      ...
      const proposal = { parent_task_id: "standard-parent", parent_changeset_id: "standard-parent-cs",
        parent_disposition_event_id: parent.event_id, parent_round: 1,
        parent_candidate_sha: manifest.digest, authorized_paths: candidate.paths,
        child: { task_id: "reviewed-t2", changeset_id: "reviewed-t2-cs", tier: "T2",
          budget: "one exact handoff", authorized_paths: candidate.paths } };
      const anchor = { kind: "standard_disposition", event_id: parent.event_id,
        candidate_sha: manifest.digest };
      const review = recordAggregateProcessReview({ type: "aggregate_v2", kind: "process_review",
        task_id: "standard-parent", changeset_id: "standard-parent-cs", reviewer_role: "frontier",
        purpose: "legacy_handoff", anchor, proposed_transition: { ...proposal, policy_version: 4 },
        review_evidence: "reviewed this exact handoff", zoom_out: "one bounded child",
        ruling, bounded_scope: "one handoff", closure_evidence: "bounded" },
      options(ctx.dir));
      assert.equal(review.ok, true, review.state);
      const rows = loadRepairEventsForProject(ctx.dir).aggregate_events;
      const oldReview = stamped({ ...rows.at(-1).event, policy_version: policy,
        transition_sha256: aggregateTransitionSha256("legacy_handoff", { ...proposal,
          policy_version: policy }) });
      writeFileSync(ledger, `${JSON.stringify(parent)}\n${JSON.stringify(oldReview)}\n`);
      const handoff = { type: "aggregate_v2", kind: "legacy_handoff", task_id: "standard-parent",
        changeset_id: "standard-parent-cs", ...proposal, owner_evidence: "Owner explicitly authorizes T2" };
      const before = readFileSync(ledger, "utf8");
      assert.equal(recordAggregateLegacyHandoff(handoff, options(ctx.dir)).ok, false,
        `v${policy} exact review requires its permitted citation`);
      assert.equal(readFileSync(ledger, "utf8"), before);
      assert.equal(recordAggregateLegacyHandoff({ ...handoff,
        child: { ...handoff.child, budget: "changed transition" },
        process_review_event_id: oldReview.event_id }, options(ctx.dir)).ok, false,
      "a changed handoff cannot cite the old exact review");
      const cited = recordAggregateLegacyHandoff({ ...handoff,
        process_review_event_id: oldReview.event_id }, options(ctx.dir));
      assert.equal(cited.ok, true, cited.state);
      const accepted = loadRepairEventsForProject(ctx.dir).aggregate_events;
...
test("a v3 typed child review remains binding when a planted successor uses v4", () => {
  const ctx = repo();
  try {
    const candidate = commit(ctx.dir, 1);
    const panel = acceptedV3Root(ctx, candidate);
    ...
    const input = { type: "aggregate_v2", kind: "child_continuation", task_id: "task-1",
      ...
      continuation_kind: "new_changeset", owner_evidence: "Owner authorizes the child",
      children: [{ task_id: "v3-reviewed-child", changeset_id: "v3-reviewed-child-cs", tier: "T2",
        budget: "one exact successor", authorized_paths: candidate.paths }] };
    const parent = derive(ctx);
    const review = recordAggregateProcessReview({ type: "aggregate_v2", kind: "process_review",
      task_id: "task-1", changeset_id: "cs-1", reviewer_role: "frontier",
      purpose: "child_continuation", proposed_transition: { policy_version: 3,
        authority_route: "owner", action_screen: _DEFAULT_CONTINUATION_SCREEN, ...input },
      anchor: { kind: "aggregate_terminal", event_id: stopped.event_id,
        panel_open_event_id: parent.panels_open[0].event_id,
        frozen_commit: candidate.commit, frozen_tree: candidate.tree },
      review_evidence: "reviewed one successor", zoom_out: "one bounded child",
      ruling: "successor", bounded_scope: "one exact child", closure_evidence: "bounded" },
    options(ctx.dir));
    assert.equal(review.ok, true, review.state);
    const ledger = repairLedgerPath(ctx.dir);
    const before = readFileSync(ledger, "utf8");
    assert.equal(recordAggregateChildContinuation(input, options(ctx.dir)).ok, false);
    assert.equal(readFileSync(ledger, "utf8"), before);
    const cited = recordAggregateChildContinuation({ ...input,
      process_review_event_id: review.event_id }, options(ctx.dir));
    assert.equal(cited.ok, true, cited.state);
    const rows = loadRepairEventsForProject(ctx.dir).aggregate_events;
    const planted = stamped({ ...rows.at(-1).event, policy_version: 4,
      process_review_event_id: null });
    writeFileSync(ledger, `${[...rows.slice(0, -1), planted].map(JSON.stringify).join("\n")}\n`);
    assert.equal(derivePendingLineageBudgets(loadRepairEventsForProject(ctx.dir).aggregate_events)
      .some((entry) => entry.task_id === "v3-reviewed-child"), false,
    "a v4 replay cannot discard the v3 successor review");
  } finally { ctx.cleanup(); }
});

for (const ruling of ["owner_decision", "successor"]) {
  test(`accepted v3 T2 ${ruling} dispatch review holds both recording and replay`, () => {
    const ctx = repo();
    try {
      const candidate = commit(ctx.dir, 1);
      const panel = acceptedV3Root(ctx, candidate);
      ...
      const decided = decide(ctx, closed, { accepted: ["F1"] });
      assert.equal(decided.ok, true, decided.state);
      const review = processReview(ctx, closed.event_id, candidate, ruling);
      assert.equal(review.ok, true, review.state);
      assert.equal(loadRepairEventsForProject(ctx.dir).aggregate_events.at(-1).event.policy_version, 3);
      mkdirSync(path.join(ctx.dir, "briefs"), { recursive: true });
      const briefPath = "briefs/round-2.md";
      const briefBytes = Buffer.from("one bounded repair\n");
      writeFileSync(path.join(ctx.dir, briefPath), briefBytes);
      const declaration = { aggregate_controller: "aggregate_v2", task_id: "task-1",
        changeset_id: "cs-1", disposition_event_id: decided.event_id,
      ...
      for (const reviewId of [null, review.event_id]) {
        const attempt = confirmRepairBrief({ declaration: { ...declaration,
          process_review_event_id: reviewId }, brief_path: briefPath }, options(ctx.dir));
        assert.equal(attempt.state, "aggregate-process-review-required",
          `${ruling} cannot authorize a dispatch with or without its citation`);
        assert.equal(readFileSync(ledger, "utf8"), standing, "a denied dispatch appends nothing");
      }
      for (const policyVersion of [3, 4]) {
        const planted = stamped({ type: "aggregate_v2", policy_version: policyVersion,
          kind: "dispatch", task_id: "task-1", changeset_id: "cs-1",
          recorded_at: `2099-01-01T00:00:0${policyVersion}.000Z`, session_id: "planted-dispatch",
          ...
          brief_sha256: createHash("sha256").update(briefBytes).digest("hex"),
          brief_size: briefBytes.length });
        writeFileSync(ledger, `${JSON.stringify(planted)}\n`, { flag: "a" });
        const state = derive(ctx);
        assert.equal(state.process_reviews.some((row) => row.event_id === review.event_id), true,
          "the typed v3 review remains accepted history");
        assert.equal(state.active_dispatch, null,
          `a planted v${policyVersion} dispatch cannot discard the reviewed hold`);
      }
    } finally { ctx.cleanup(); }
  });
}

test("accepted v3 T2 finish review still authorizes its exact R2 dispatch", () => {
  const ctx = repo();
  try {
    const first = commit(ctx.dir, 1);
    const panel = acceptedV3Root(ctx, first);
    ...
    const authority = dispatchBatch(ctx, decided.event_id, closed.event_id, 2, null,
      { processReviewId: review.event_id });
    const loaded = loadRepairEventsForProject(ctx.dir);
    assert.equal(loaded.aggregate_events.find((row) => row.event_id === authority.dispatch)
      .event.policy_version, 3);
    const next = commit(ctx.dir, 2);
    const r2 = openPanel(ctx, 2, next, { incoming: { dispatch: authority.dispatch,
      worker: authority.worker } });
    assert.equal(r2.opened.ok, true, r2.opened.state);
    assert.equal(derive(ctx).panels_open.at(-1).event_id, r2.opened.event_id);
  } finally { ctx.cleanup(); }
});
...
test("H2: close eligibility against the FULL admitted set — verification, handoff replacement, and the pre-mint window, each with its positive control", () => {
  ...
  try {
    ...
    // program and erasing its successors: a measured fail-open, executed.
    const loaded = loadRepairEventsForProject(ctx.dir);
...
// The measured grind walks, labelled by their finding ids — WALK-F1's compliant refreeze/
// roster/scope grind and WALK-F2's close-restart, carried as the V1/V2 walks. Each step of
// each walk must refuse exactly as recorded.
...
test("M35: the reservation lift is COVERAGE-SCOPED — a child's GO releases its own budget, never its siblings' surfaces", async () => {
  ...
  try {
    ...
    // set) and the y-open lands — the measured hole, four independent reproductions.
    const loaded = loadRepairEventsForProject(ctx.dir);
    ...
    "the whole-union lift reproduces the measured hole: one narrow GO releases the sibling's surface");
  } finally { ctx.cleanup(); rmSync(mutantDir, { recursive: true, force: true }); }
});
...
test("M37: a no-disposition close anchors its successor at the WINNING OPEN — ground carried, one successor per anchor", async () => {
  ...
  try {
    ...
    // does not exist — a measured finding).
    const ownerClose = recordAggregateClose({
      type: "aggregate_v2", kind: "close", task_id: "task-1", changeset_id: "cs-1",
      disposition_event_id: null, panel_open_event_id: panel.opened.event_id,
      reason: "abandoned after collection", owner_evidence: "Owner keyboard",
    ...
    // child's open both evaporate on replay — the reservation is un-exitable again (a measured deadlock).
    const loaded = loadRepairEventsForProject(ctx.dir);
...
test("M39: a PENDING lineage budget binds from declaration to first open — snipes refused, collisions refused at declaration", async () => {
  ...
  try {
    ...
    // the surface — disabling it reproduces the measured snipe. Post-open, the same disabled arm
    ...
    "without the pending arm the pre-open snipe lands — the measured lockout of the child");
...
test("M40: an ACTIVE program binds its authorized set PLUS any undisposed open — and EXACTLY the authorized set before one opens", async () => {
  ...
  // remediating its own reviewed surface (an executed defect).
  const ctx = repo();
  const mutantDir = mkdtempSync(path.join(os.tmpdir(), "breaker-mutants-"));
  try {
    writeFileSync(path.join(ctx.dir, "src", "y.mjs"), "export const y = 1;\n");
    ...
    // Disabled arm: bind only the authorized set (the prior form) and the wedge lands.
    const loaded = loadRepairEventsForProject(ctx.dir);
    ...
    assert.equal(mutant.deriveAggregateRepairState([...loaded.aggregate_events, plantedWedge], "planted-p2",
      { standardEvents: loaded.events }).panels_open.length, 1,
    "binding only the authorized set reproduces the measured wedge");
  } finally { ctx.cleanup(); rmSync(mutantDir, { recursive: true, force: true }); }
...
// ── terminal-round-breaker-2 · the M41–M47 pins ─────────────────────────────────────────────────
...
test("M41: the lift is budget ∩ OPENED COVERAGE — a wide budget is a plan, not a repair", async () => {
  ...
  try {
    ...
    // Disabled arm: lift the whole declared budget without the repaired.has filter — the measured
    ...
    assert.equal(mutant.deriveAggregateRepairState([...loaded.aggregate_events, plantedY], "m41-planted",
      { standardEvents: loaded.events }).panels_open.length, 1,
    "lifting the un-reviewed budget remainder reproduces the measured gap");
  } finally { ctx.cleanup(); rmSync(mutantDir, { recursive: true, force: true }); }
});
...
test("M42: the reservation exception walks the FULL ancestor chain — a grandchild is not locked out by its grandparent", async () => {
  ...
  try {
    ...
    // evaporates on replay — the measured nested lockout, the exit lattice one-shot again.
    const loaded = loadRepairEventsForProject(ctx.dir);
...
test("M43: ONE LIVE continuation per anchor — a stranded virgin child reopens the anchor; a GO child does not", async () => {
  ...
  try {
    ...
    // and the accepted re-continuation evaporates on replay — the measured stranded-slice brick.
    const loaded = loadRepairEventsForProject(ctx.dir);
...
test("M44: pending-vs-active refuses AT DECLARATION — continuation and legacy handoff alike — and the pending hold is NAMED in the deny", async () => {
  ...
  try {
    ...
    "without the declaration-time check the wedge lands — the measured mutual brick");
...
test("M45: the CLOSED floor carries the UNDISPOSED GROUND — collected-but-unadjudicated harms ride the lineage", async () => {
  ...
  try {
    ...
    // the floor and the H1-only successor lands with its child, shedding H9 — the measured shed.
    ...
    assert.equal(mutant.deriveAggregateRepairState(rows, "c45-shed", { standardEvents: window.events })
      .panels_open.length, 1,
    "without the ground half of the floor, the H1-only successor sheds H9 — the measured shed");
  } finally { ctx.cleanup(); rmSync(mutantDir, { recursive: true, force: true }); }
});
...
test("M47: the trigger cap is the exact-carry bound — a 101-id ground ACCEPTS, 2601 ids refuse on shape", () => {
  ...
  try {
    ...
    // ONE unified malformed spelling. 1201 now ACCEPTS: the executed shape carried a 1300-id
...
test("M48: THE CROSS-ROUND BASE PIN — every later round re-derives from ROUND 1's base, and a refreeze may not move it", async () => {
  ...
  const mutantDir = mkdtempSync(path.join(os.tmpdir(), "breaker-mutants-"));
  try {
    // Round 1 reviews a candidate touching BOTH x and y, accepts F1, and dispatches the y repair.
    writeFileSync(path.join(ctx.dir, "src", "y.mjs"), "export const y = 1;\n");
    ...
    // The worker repairs y on top of round 1's freeze. TWO honest-looking declarations of the SAME
    // round-2 candidate exist: from round 1's own base (the whole candidate: x AND y), or from round 1's
    ...
    // A real ref at round 1's freeze, so the moved-base declaration is fully WELL-FORMED: its
    ...
    // the measured finding, the deepest of the set: "immutable base..frozen pair" was claimed but
    // never enforced ACROSS rounds, so a moved base reviewed only the last delta while the GO (and the
...
test("M49: CAP TRUTH AT THE EXECUTED SHAPE — the executed-shape 1300-id STOP carries, and the ledger stays derivable", async () => {
  const ctx = repo();
  const mutantDir = mkdtempSync(path.join(os.tmpdir(), "breaker-mutants-"));
  try {
    // The executed-shape reproduction: 12 seats × 100 raw ids (both grammar maxima at once) plus
...
test("M52: the pending-hold diagnosis survives a NON-free-seat-first roster — the executed non-free-seat-first shape", async () => {
  const ctx = repo();
  const mutantDir = mkdtempSync(path.join(os.tmpdir(), "breaker-mutants-"));
  try {
    ...
    // the executed roster looked like this.
    ...
    // shape the executed panel ran, and the operator gets a bare conflict naming nothing.
    const mutant = await importMutant(mutantDir, [[
...
// ── batch 5 · the handoff-guard pins ─────────────────────────────────────────────────────────────
...
// ── batch 6 · the remainder-guarantee doc-truth pins (no controller change) ──────────────────────

test("M55: THE HONEST GUARANTEE — a budget SHARING the remainder is admitted (reviewed no-op or repair); a budget sharing NOTHING refuses; the surface is never released unreviewed", async () => {
  const ctx = repo();
  const mutantDir = mkdtempSync(path.join(os.tmpdir(), "breaker-mutants-"));
  try {
    // WHAT THIS PINS, AND WHAT IT DELIBERATELY DOES NOT. The free seat walked a 5-cycle "grind":
    // a successor whose budget NAMES the remainder but whose child then OPENS only unrelated paths
    // reopens the anchor again and again. That is NOT a brick and NOT an escape — every such
    // successor is a fully-gated, fully-reviewed program that lifts NOTHING it did not open, so the
    // reserved surface is never surrendered without a GO child that actually opened it. The model
    // already sanctions unbounded REVIEWED successors; building opened-coverage machinery to stop a
    // reviewed no-op would be exactly the repair-introduced state-growth the root replacement
    // forbids. So the shipped guarantee — the one pinned here — is the WEAKER, TRUE one:
    //   · a re-continuation whose budget shares NO path with the un-lifted remainder REFUSES;
    //   · a re-continuation whose budget shares a remainder path is ADMITTED, and its child then
    //     does its own fully-gated work (which may be a no-op wrt the remainder);
    //   · the reservation is never lifted except by a GO child that OPENED the freed path.
    // M54 carries the disabled-arm mutant for the shipped mechanism (budget-string targeting); this
    // pin is the guarantee stated honestly, so it grows no new machinery to assert.
    const world = goHopWorld(ctx); // A STOP {x,y}; B repairs x, GOes ⇒ un-lifted remainder {y}
    const reopenA = (task, paths) => recordAggregateChildContinuation({
      type: "aggregate_v2", kind: "child_continuation", task_id: "a50", changeset_id: "a50-cs",
      parent_disposition_event_id: world.aStop.event_id, trigger_ids: ["A1"],
      continuation_kind: "new_changeset", owner_evidence: "Owner successor",
    ...
    const yReserved = (probeTask) => {
      const probe = sideCandidate(ctx, `${probeTask}-line`, { "src/y.mjs": `export const y = '${probeTask}';\n` });
      return openPanel(ctx, 1, probe, { task: probeTask, changeset: `${probeTask}-cs` }).opened.ok === false;
    };

    // POLARITY A — budget shares NOTHING with the remainder ⇒ REFUSE.
    const disjoint = reopenA("m55dis", ["docs/unrelated55.md"]);
    assert.equal(disjoint.ok, false, "a budget sharing nothing with the remainder cannot reopen");
    assert.equal(disjoint.state, "aggregate-continuation-conflict");
    assert.ok(yReserved("m55probe0"), "y is reserved before any successor runs");

    // POLARITY B, and the honest heart of it: FIVE successive budget-SHARES-remainder successors,
    // each opening ONLY an unrelated path (budget {y, uN}, opens {uN}), each running to GO. Every
    // one is ADMITTED — the reviewed no-op the free seat called a grind — and after all five, y is
    // STILL reserved. Unbounded gated successors, zero surface released.
    for (let cycle = 1; cycle <= 5; cycle += 1) {
      const uPath = `src/u55_${cycle}.mjs`;
      const child = `m55c${cycle}`;
      const cont = reopenA(child, [uPath, "src/y.mjs"].sort()); // budget SHARES y AND names an off-path
      assert.equal(cont.ok, true,
        `cycle ${cycle}: a budget sharing the remainder is admitted (a reviewed successor): ${cont.state}`);
      const cand = sideCandidate(ctx, `${child}-line`, { [uPath]: `export const u = ${cycle};\n` });
      const open = openPanel(ctx, 1, cand, { task: child, changeset: `${child}-cs`,
        lineage: { continuation: cont.event_id } });
      assert.equal(open.opened.ok, true, `cycle ${cycle}: the child opens its off-path: ${open.opened.state}`);
      const closed = closePanel(ctx, open, cand, [], { task: child, changeset: `${child}-cs` });
      const go = decide(ctx, closed, { task: child, changeset: `${child}-cs`,
        terminal_state: "GO", remediation_kind: null, authorized_paths: [] });
      assert.equal(go.ok, true, `cycle ${cycle}: the child GOes on work it actually did: ${go.state}`);
      assert.equal(derive(ctx, child).terminal, "GO");
      assert.ok(yReserved(`m55probe${cycle}`),
        `cycle ${cycle}: the GO opened ${uPath}, not y — y is NEVER released without review`);
    }

    // After five reviewed no-ops the anchor is still reopenable (remainder survives), and a
    // disjoint budget STILL refuses — the guarantee is stable across the whole "grind".
    assert.equal(reopenA("m55dis2", ["docs/unrelated55b.md"]).ok, false,
      "the disjoint refusal holds no matter how many reviewed successors ran");
    // …and the honest exit: a successor that actually OPENS y and GOes DOES lift it.
    const repair = reopenA("m55fix", ["src/y.mjs"]);
    assert.equal(repair.ok, true, `a remainder-sharing successor is admitted: ${repair.state}`);
    const fixCand = sideCandidate(ctx, "m55fix-line", { "src/y.mjs": "export const y = 'fixed';\n" });
    const fixOpen = openPanel(ctx, 1, fixCand, { task: "m55fix", changeset: "m55fix-cs",
      lineage: { continuation: repair.event_id } });
    assert.equal(fixOpen.opened.ok, true, fixOpen.opened.state);
    const fixClosed = closePanel(ctx, fixOpen, fixCand, [], { task: "m55fix", changeset: "m55fix-cs" });
    const fixGo = decide(ctx, fixClosed, { task: "m55fix", changeset: "m55fix-cs",
      terminal_state: "GO", remediation_kind: null, authorized_paths: [] });
    assert.equal(fixGo.ok, true, fixGo.state);
    assert.equal(openPanel(ctx, 1,
      sideCandidate(ctx, "m55out-line", { "src/y.mjs": "export const y = 'out';\n" }),
      { task: "m55out", changeset: "m55out-cs" }).opened.ok, true,
    "only a GO child that OPENED y releases y — review is the price of release, every time");
  } finally { ctx.cleanup(); rmSync(mutantDir, { recursive: true, force: true }); }
});

test("F2: the base-pin refusal is DIAGNOSED — a moved-base open names round 1's base and both exits; a pinned-base conflict carries no base detail; terminality wins the ordering", async () => {
  const ctx = repo();
  try {
    // Round 1 reviews x+y, CONTINUEs, dispatches, and a worker is admitted — so the entry chain is
    // complete and any later refusal is NOT aggregate-worker-required.
    writeFileSync(path.join(ctx.dir, "src", "y.mjs"), "export const y = 1;\n");
    const r1 = commit(ctx.dir, "pF2");
    assert.deepEqual(r1.paths, ["src/x.mjs", "src/y.mjs"]);
    const panel = openPanel(ctx, 1, r1);
    assert.equal(panel.opened.ok, true, panel.opened.state);
    const closed = closePanel(ctx, panel, r1, ["F1"]);
    ...
    // The round-2 repair candidate, plus a real ref at round 1's freeze so the moved-base declaration
    // is fully well-formed and only the PIN can refuse it.
    writeFileSync(path.join(ctx.dir, "src", "y.mjs"), "export const y = 2;\n");
    execFileSync("git", ["add", "src/y.mjs"], { cwd: ctx.dir }); // NOT -A: the brief stays untracked
    execFileSync("git", ["commit", "-qm", "r2-repair"], { cwd: ctx.dir });
    const r2 = {
    ...
    execFileSync("git", ["update-ref", "refs/remotes/origin/f2line", r1.commit], { cwd: ctx.dir });

    // MOVED BASE ⇒ the refusal NAMES round 1's base ref, its (truncated) commit, and BOTH exits
    // (continue on the original base · close and declare a successor).
    const moved = openPanel(ctx, 2, { commit: r2.commit, tree: r2.tree, paths: ["src/y.mjs"] },
      { incoming: { dispatch: authority.dispatch, worker: authority.worker },
        baseRef: "origin/f2line", baseCommit: r1.commit });
    assert.equal(moved.opened.ok, false, "the moved base refuses");
    assert.equal(moved.opened.state, "aggregate-panel-open-conflict");
    assert.match(moved.opened.detail ?? "", /pinned to round 1's/, "the detail names the pin");
    assert.match(moved.opened.detail ?? "", /origin\/main/, "…and round 1's base REF");
    assert.match(moved.opened.detail ?? "", new RegExp(ctx.base.slice(0, 12)),
      "…and round 1's base COMMIT (truncated)");
    assert.match(moved.opened.detail ?? "", /continue on the original base/, "…and exit one");
    assert.match(moved.opened.detail ?? "", /close and declare a successor/, "…and exit two");

    // PINNED BASE, other conflict ⇒ NO base detail. The incoming refs are swapped (both ID64,
    // both wrong-role), so the row refuses on the incoming-authority mismatch while its base
    // matches round 1's — the base-pin branch must NOT fire.
    const pinnedConflict = openPanel(ctx, 2, { commit: r2.commit, tree: r2.tree, paths: ["src/x.mjs", "src/y.mjs"] },
      { incoming: { dispatch: authority.worker, worker: authority.dispatch } });
    assert.equal(pinnedConflict.opened.ok, false, "the swapped incoming refs refuse");
    assert.equal(pinnedConflict.opened.state, "aggregate-panel-open-conflict");
    assert.doesNotMatch(pinnedConflict.opened.detail ?? "", /pinned to round 1's/,
      "a pinned-base conflict carries no base detail — the base is not the cause");

    // ORDERING: terminality is diagnosed BEFORE the base pin. Re-run the whole world to a STOP,
    // then attempt a moved-base open — a base-pin-first ordering would print the base detail; the
    // shipped ordering returns aggregate-terminal.
    const ctx2 = repo();
    try {
      writeFileSync(path.join(ctx2.dir, "src", "y.mjs"), "export const y = 1;\n");
      const s1 = commit(ctx2.dir, "pF2s");
      const sp = openPanel(ctx2, 1, s1);
      assert.equal(sp.opened.ok, true, sp.opened.state);
      const sc = closePanel(ctx2, sp, s1, ["S1"]);
      const stop = decide(ctx2, sc, { accepted: ["S1"], terminal_state: "STOP",
        remediation_kind: null, authorized_paths: [] });
      assert.equal(stop.ok, true, stop.state);
      writeFileSync(path.join(ctx2.dir, "src", "y.mjs"), "export const y = 9;\n");
      execFileSync("git", ["add", "src/y.mjs"], { cwd: ctx2.dir });
      execFileSync("git", ["commit", "-qm", "s2"], { cwd: ctx2.dir });
      const s2 = {
        commit: execFileSync("git", ["rev-parse", "HEAD"], { cwd: ctx2.dir, encoding: "utf8" }).trim(),
        tree: execFileSync("git", ["rev-parse", "HEAD^{tree}"], { cwd: ctx2.dir, encoding: "utf8" }).trim(),
      };
      execFileSync("git", ["update-ref", "refs/remotes/origin/f2sline", s1.commit], { cwd: ctx2.dir });
      const movedOnTerminal = openPanel(ctx2, 2, { commit: s2.commit, tree: s2.tree, paths: ["src/y.mjs"] },
        { baseRef: "origin/f2sline", baseCommit: s1.commit });
      assert.equal(movedOnTerminal.opened.ok, false);
      assert.equal(movedOnTerminal.opened.state, "aggregate-terminal",
        "terminality is diagnosed before the base pin — a moved base on a STOPped program is aggregate-terminal");
      assert.equal(movedOnTerminal.opened.terminal, "STOP");
      assert.doesNotMatch(movedOnTerminal.opened.detail ?? "", /pinned to round 1's/,
