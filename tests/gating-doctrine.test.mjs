...
// ------------------------------------------- the predicate cure, duplicated ON PURPOSE and pinned

test("the predicate→DECLARATION/SENSOR cure is byte-equal on BOTH surfaces that carry it", () => {
  // WHY THIS DUPLICATION IS DELIBERATE, and why a pointer would not do. `core/README.md` § Do NOT
  // "read everything in core/" states that `INVARIANTS.md` is shipped to the REVIEWER by the gate
  // runner and is "never read by the builder at all". So the two audiences are disjoint in opposite
  // directions: the cure is design guidance a BUILDER needs (and a builder reads FOUNDATIONS at boot,
  // never INVARIANTS), while the REVIEWER judging whether a control should have been a predicate
  // receives only the payload and cannot fetch a doc that is not in it. A pointer therefore dangles
  // for whichever reader cannot follow it — the shape that ships a broken citation in every adopter.
  //
  // Unbound duplication is the defect this repo has already paid for (one doctrine narrated on four
  // surfaces, hand-synchronised, missed every round). BOUND duplication is a design choice. This test
  // is the binding: edit one copy and this reddens.
  //
  // Whitespace-flattened, because the two copies are typed differently by necessity — INVARIANTS
  // carries it as an indented list continuation, FOUNDATIONS as one long line. Comparing typography
  // instead of the claim would make this pin fire on a reflow and miss a real divergence.
  const claim = (rel, otherDoc) => {
    const flat = read(rel);
    const start = flat.indexOf("When a control needs a JUDGMENT");
    assert.notEqual(start, -1, `${rel}: the cure is MISSING — it must appear on both surfaces`);
    // Ends at the duplication marker, which necessarily names the OTHER file and so cannot be shared.
    const end = flat.indexOf("*(This paragraph is duplicated verbatim", start);
    assert.notEqual(end, -1, `${rel}: the duplication marker is gone — a maintainer will now "fix" this by deleting one copy`);
    assert.match(flat.slice(end, end + 400), new RegExp(otherDoc.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
      `${rel}: the marker must name the other surface by path, or it points at nothing`);
    // The access-model reason must travel WITH the duplication. Without it the next maintainer reads
    // two copies as sloppiness and deletes one, silently blinding whichever reader lost its copy.
    assert.match(flat.slice(end, end + 400), /never reads (this|that) file .*only the payload/,
      `${rel}: the marker must state WHY, not merely that it is duplicated`);
    return flat.slice(start, end).trim();
  };
  const inv = claim("core/INVARIANTS.md", "core/FOUNDATIONS.md");
  const fnd = claim("core/FOUNDATIONS.md", "core/INVARIANTS.md");
  assert.equal(inv, fnd,
    "the cure has DIVERGED between core/INVARIANTS.md rule 1 and core/FOUNDATIONS.md P2 — " +
    "one was edited and the other was not, which is exactly what this pin exists to catch");
  // Anti-vacuity: a pin comparing two empty strings passes forever. Assert the claim is really there.
  assert.ok(inv.length > 400, "the pinned claim is suspiciously short — re-point this test");
  for (const re of [/DECLARATION/, /SENSOR/, /deliberate false statement rather than a parsing accident/,
                    /surfaces candidates and never claims prevention/, /do not build generation four/])
    assert.match(inv, re, `the cure lost a load-bearing clause: ${re}`);
  // The distinction from OPERATE's "sensors, not actuators" is load-bearing: conflating them is the
  // ported-verbatim-is-not-safe trap, so it must survive on both surfaces too.
  assert.match(inv, /governs whether a check ENACTS, not whether it can decide/);
});
